...
frappe.ui.form.on('Bank Sync', {
	...
	refresh: function(frm) {
		...
		frm.add_custom_button(__('Import Data from camt-File'), () => {
            frappe.prompt({
				...
				frappe.call({
					method: 'camt_import.camt_import.doctype.bank_sync.bank_sync.import_camt',
					args: {
						file: values.file,
	...
	route_to_bank_statement_import(frm) {
		...
		frappe.call({
			method: "erpnext.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.upload_bank_statement",
			args: {
				dt: frm.doc.doctype,
				dn: frm.doc.name,
				company: frm.doc.company,
